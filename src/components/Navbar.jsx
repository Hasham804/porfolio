...
import { FiverrIcon, GithubIcon, LinkedInIcon, MoonIcon,  UpworkIcon } from "./icon"
...
const CustomMobileLink = ({ href, title, className = "" , toggle}) => {
  ...
  return (
    <button href={href} className={`${className} relative group text-light dark:text-dark my-2`} onClick={handlecli}>
      {title}
    ...
    <header className=' w-full px-32 py-8 font-medium flex items-center justify-between dark:text-light relative z-10 lg:px-16 md:px-12 sm:px-8'>
      ...
      <div className='w-full flex justify-between items-center lg:hidden'>
        ...
        <nav className='flex items-center justify-center flex-wrap'>
            ...
            <motion.a href={"https://www.upwork.com/freelancers/~010da425b07a99f22a"} target='_blank'
           whileHover={{y:-2}}
            whileTap={{scale:0.9}}
            className='w-6 mx-3'><UpworkIcon/></motion.a>
            <motion.a href={"/"} target='_blank' 
            whileHover={{y:-2}}
            whileTap={{scale:0.9}}
            className='w-6 ml-3'><FiverrIcon/></motion.a>
      ...
      animate={{scale:1, opacity:1}}
       ...
       className='min-w-[70vw] flex flex-col justify-between items-center fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32'>
        ...
        <nav className='flex items-center justify-center flex-wrap mt-2'>
           ...
           whileHover={{y:-2}}
            whileTap={{scale:0.9}}
            className='w-6 mx-3 sm:mx-1'><UpworkIcon/></motion.a>
            <motion.a href={"/"} target='_blank' 
            whileHover={{y:-2}}
            whileTap={{scale:0.9}}
            className='w-6 ml-3 sm:mx-1'><FiverrIcon/></motion.a>
